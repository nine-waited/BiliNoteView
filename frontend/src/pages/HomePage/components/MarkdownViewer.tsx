  ...
  return {
    ...
    a: ({ href, children, ...props }: any) => {
      ...
      if (isOriginLink) {
        ...
        return (
          <span className="origin-link my-2 inline-block max-w-full">
            <a
              href={href}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex max-w-full items-center gap-1.5 rounded-full bg-blue-50 px-3 py-1 text-sm font-medium text-blue-700 transition-colors hover:bg-blue-100"
              {...props}
            >
      ...
      if (href?.startsWith('#')) {
        ...
        return (
          <a
            ...
            className="text-primary hover:text-primary/80 break-words font-medium underline underline-offset-4"
            {...props}
          >
            {children}
          </a>
      ...
      return (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="text-primary hover:text-primary/80 inline break-words font-medium underline underline-offset-4"
          {...props}
        >
          {children}
    ...
    ol: ({ children, ...props }: any) => (
      <ol className="my-6 list-decimal [&>li]:mt-2" {...props}>
        {children}
      </ol>
    ),
    ...
    code: ({ inline, className, children, ...props }: any) => {
      ...
      if (!inline && match) {
        return (
          <div className="group bg-muted relative my-6 max-w-full overflow-x-auto rounded-lg border shadow-sm">
      ...
      return (
        <code
          className="bg-muted relative break-words rounded px-[0.3rem] py-[0.2rem] font-mono text-sm"
          {...props}
        >
          {children}
        </code>
    ...
    table: ({ children, ...props }: any) => (
      <div className="my-6 w-full max-w-full overflow-auto">
    ...
    th: ({ children, ...props }: any) => (
      <th
        className="border-muted-foreground/20 break-words border px-4 py-2 text-left font-medium [&[align=center]]:text-center [&[align=right]]:text-right"
        {...props}
      >
        {children}
      </th>
    ...
    td: ({ children, ...props }: any) => (
      <td
        className="border-muted-foreground/20 break-words border px-4 py-2 text-left [&[align=center]]:text-center [&[align=right]]:text-right"
        {...props}
      >
        {children}
      </td>
...
const MarkdownViewer: FC<MarkdownViewerProps> = memo(({ status, appMode = 'default' }) => {
  ...
  const [transcriptSource, setTranscriptSource] = useState<'subtitle' | 'whisper' | undefined>()
  // 确保baseURL没有尾部斜杠
  ...
  useEffect(() => {
    ...
    if (!isMultiVersion) {
      ...
      setPromptTokens(currentTask.tokenUsage?.prompt_tokens ?? 0)
      setCompletionTokens(currentTask.tokenUsage?.completion_tokens ?? 0)
      setTotalTokens(currentTask.tokenUsage?.total_tokens ?? 0)
      setTranscriptSource(currentTask.tokenUsage?.transcript_source)
  ...
  useEffect(() => {
    ...
    if (currentVer) {
      ...
      setPromptTokens(currentVer.prompt_tokens ?? 0)
      setCompletionTokens(currentVer.completion_tokens ?? 0)
      setTotalTokens(currentVer.total_tokens ?? 0)
      setTranscriptSource(
        currentVer.transcript_source ??
          (['ai-zh', 'zh-cn', 'zh-hans'].includes(
            String(currentTask?.transcript?.language || '').toLowerCase()
          )
            ? 'subtitle'
  ...
  if (status === 'loading') {
    return (
      <div className="flex h-full min-h-0 w-full flex-col items-center justify-center space-y-4 text-neutral-500">
      ...
      <div className="flex h-full min-h-0 w-full flex-col items-center justify-center space-y-3 text-neutral-500">
        ...
        <div className="text-center">
          <p className="text-lg font-bold">
            {appMode === 'cloud' ? '从左侧选择一篇笔记' : '输入视频链接并点击"生成笔记"'}
          </p>
  ...
  if (status === 'failed' && !isMultiVersion && appMode !== 'cloud') {
    return (
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col overflow-hidden">
      <MarkdownHeader
        ...
        showChat={appMode === 'cloud' ? false : showChat}
        setShowChat={appMode === 'cloud' ? undefined : setShowChat}
        viewMode={viewMode}
      ...
      {viewMode === 'map' ? (
        ...
        <div className="flex min-h-0 flex-1 gap-2 overflow-hidden bg-white py-2">
          {selectedContent && selectedContent !== 'loading' && selectedContent !== 'empty' ? (
            <>
              {appMode !== 'cloud' && showChat === 'full' && currentTask ? (
                <div className="h-full min-h-0 w-full">
                  <ChatPanel taskId={currentTask.id} mode="full" onModeChange={setShowChat} />
                </div>
              ) : (
              <>
              <div className="note-preview-scroll min-h-0 min-w-0 flex-1 basis-0 overflow-x-hidden overflow-y-auto">
                ...
                <div className="markdown-body note-markdown-content box-border w-full max-w-full px-3 pr-4 pb-8">
                  <ReactMarkdown
              ...
              {showTranscribe && (
                <div className="ml-2 min-h-0 min-w-[220px] max-w-[42%] flex-[0_0_38%]">
                  <TranscriptViewer />
                </div>
              )}
              {/* 侧边问答模式：markdown + ChatPanel 各占一半 */}
              {appMode !== 'cloud' && showChat === 'half' && currentTask && (
                <div className="ml-2 min-h-0 min-w-[220px] max-w-[42%] flex-[0_0_38%]">
