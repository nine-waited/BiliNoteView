...
import { isTaskFinished, useTaskStore } from '@/store/taskStore'
...
const NoteForm = () => {
  ...
  /* ---- 全局状态 ---- */
  const { submitGenerateTask, currentTaskId, setCurrentTask, getCurrentTask, isSubmitting } =
    useTaskStore()
  ...
  /* ---- 帮助函数 ---- */
  const isGenerating = () => !isTaskFinished(getCurrentTask()?.status)
  const generating = isGenerating() || isSubmitting
  ...
  const onSubmit = async (values: NoteFormValues) => {
    ...
    const result = await submitGenerateTask(payload)
    if (
      !result.ok &&
      result.reason === 'transcriber_model_not_ready' &&
      !result.downloading
    ) {
